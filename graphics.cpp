...
vec3 Graphics::material_diffuse_color;
vec3 Graphics::material_specular_color;
int Graphics::lightCount = 0;
vec3 Graphics::lightColor[MAX_LIGHT_COUNT];
vec3 Graphics::lightPosition[MAX_LIGHT_COUNT];
...
void Graphics::Cube(vec3 position, quat rotation, vec3 scale) {
  ...
  Graphics::world_matrix = glm::translate(mat4(), position) *
                           glm::toMat4(rotation) * glm::scale(mat4(), scale);
  RenderToCamera([]() { base_cube_mesh->Render(); });
}

void Graphics::SetMaterial(vec3 diffuse_color, vec3 specular_color) {
...
void Graphics::SetMaterial(vec3 diffuse_color) {
    Graphics::SetMaterial(diffuse_color, diffuse_color);
}

void Graphics::PointLight(vec3 position, vec3 color, float intensity) {
  Graphics::lightColor[Graphics::lightCount % MAX_LIGHT_COUNT] = color * intensity;
