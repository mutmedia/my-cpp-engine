...
EventSystem::EventSystem() { permanent_events_.clear(); }
...
void EventSystem::Add(Event e, Callback c) { AddNew(e, c, EVENT_PERMANENT); }

void EventSystem::Fire(Event e) {
  auto find = permanent_events_.find(e);
  if (find == permanent_events_.end())
    return;
  std::for_each(permanent_events_[e].begin(), permanent_events_[e].end(),
                [](Callback c) { c(); });
}
